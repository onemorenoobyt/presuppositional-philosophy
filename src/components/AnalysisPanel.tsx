...
const AnalysisPanel: React.FC = () => {
  ...
  const proofTerm = useMemo(() => {
    ...
    const proofIdPattern = activeTerm.id.replace(/teorema|proposicion|conjetura|corolario|lema/, 'demostracion');
    // Busca cualquier término que coincida con el patrón de demostración
    for (const term of termsMap.values()) {
        if (term.id.startsWith(proofIdPattern)) return term;
    }
    return null;
  }, [activeTerm, termsMap]);
  ...
  useEffect(() => {
    // Cuando el término activo cambia, vuelve a la pestaña de definición por defecto.
    setActiveTab('definition');
  }, [activeTermId]);

  if (!activeTerm) {
    return <div className="p-4 h-full flex items-center justify-center"><p className="text-gray-500">Selecciona un término del índice.</p></div>;
  }
...
const TabButton: React.FC<{isActive: boolean, onClick: () => void, children: React.ReactNode}> = ({isActive, onClick, children}) => (
    <button onClick={onClick} className={`py-2 px-1 text-sm font-medium border-b-2 ${isActive ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
        {children}
    </button>
);
